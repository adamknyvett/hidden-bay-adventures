...
// Tui Ridge and Kahawai Quarters are now fully INDEPENDENT listings.
// The Harbour View Duo feed is no longer merged into either cabin —
// each cabin only reflects its own calendar.
export default async function handler(req, res) {
  ...
  const urls = {
    ...
    // duo: process.env.DUO_ICAL_URL, // no longer merged — cabins are independent
  };
  ...
  // Apply manual overrides only — no Duo merge anymore
  ...
  const tuiBlocked = mergeUnique(rawBlocked.tui, manualByCabin.tui);
  const kahBlocked = mergeUnique(rawBlocked.kah, manualByCabin.kah);
